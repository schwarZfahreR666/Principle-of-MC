...
interrupt [EXT_INT1] void INT_1 (void)
{
 ...
 void display(void)                // 6Î»LEDÊýÂë¹Ü¶¯Ì¬É¨Ãèº¯Êý 
  { 
   PORTC = 0xff;    
...
TWCR=(0<<TWEA) | (0<<TWSTA) | (0<<TWSTO) | (0<<TWEN) | (0<<TWIE); 
    ...
    TCNT0=0x83; 
     ...
     #asm("sei") 
      time[2] = 23; 
      time[1] = 58;
       time[0] = 55;        // ÉèÊ±¼ä³õÖµ23:58:55 ? posit = 0; 
        time_to_disbuffer();
...
while (1)
      ...
      // Place your code here
       ...
       if (time_1s_ok)        // 1Ãëµ½ 
        { 
         time_1s_ok = 0; 
          point_on = ~point_on;  // ÃëÏÔÊ¾±êÖ¾ 
          if (++time[0] >= 60)    // ÒÔÏÂÊ±¼äµ÷Õû 
           { 
            time[0] = 0; 
             if (++time[1] >= 60) 
              { 
               time[1] = 0; 
                if (++time[2] >= 24) time[2] = 0; 
                   ...
                   time_to_disbuffer();    // ÐÂµ÷ÕûºÃµÄÊ±¼äËÍÏÔÊ¾»º³åÇø 
                    } 
      }
}
